...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_FILES['pdf_file']['name'])) {
        ...
        if (move_uploaded_file($pdfTempName, $pdfFilePath)) {
            ...
            $sqlSelectImage = "SELECT image FROM hpsxp_documents WHERE id=?";
            ...
            if (move_uploaded_file($imageTempName, $imageFilePath)) {
                ...
                $sql = "UPDATE hpsxp_documents SET title=?, content=?, pdf_file=?, image=? WHERE id=?";
        ...
        $sql = "UPDATE hpsxp_documents SET title=?, content=? WHERE id=?";
        $stmt = mysqli_prepare($conn, $sql);
